...
class FetchRequest:
    ...
    def request(self, url, **kwargs):
        ...
        # Use the in-memory cookie jar for the request to capture new cookies
        ...
        try:
            ...
            # Thread-safe cookie saving with merge logic
            with self._cookie_lock:
                # 1. Load latest cookies from disk into a temporary jar
                temp_jar = http.cookiejar.LWPCookieJar(self.cookie_file)
                if os.path.exists(self.cookie_file):
                    try:
                        temp_jar.load(ignore_discard=True, ignore_expires=True)
                    except Exception as e:
                        if self.debug:
                            print(f"Error loading cookies during merge: {e}")

                # 2. Update temp jar with fresh cookies from this request (in self.cookie_jar)
                # self.cookie_jar contains the state *after* the request
                for cookie in self.cookie_jar:
                    temp_jar.set_cookie(cookie)

                # 3. Save the merged result back to disk
                temp_jar.save(ignore_discard=True, ignore_expires=True)
