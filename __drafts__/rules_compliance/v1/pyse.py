...
# ISOLATION: Default output to __drafts__ if not specified absolute path
...
def save_links(links, filename='results.txt'):
    ...
    # ISOLATION: Ensure filename is in DEFAULT_OUTPUT_DIR if it's just a filename
    if not os.path.isabs(filename):
        filename = os.path.join(DEFAULT_OUTPUT_DIR, filename)
