...
# ISOLATION CONSTANT
DRAFT_OUTPUT_DIR = os.path.join(os.path.dirname(__file__), 'output')

def ensure_draft_output_dir():
    if not os.path.exists(DRAFT_OUTPUT_DIR):
        os.makedirs(DRAFT_OUTPUT_DIR)

def get_isolated_path(filename):
    """Ensure output goes to the drafts folder during isolation mode."""
    ensure_draft_output_dir()
    # Check if filename is just a name or a path.
    # If absolute path, we might want to override or warn.
    # For now, just prepend if it's relative.
    if not os.path.isabs(filename):
        return os.path.join(DRAFT_OUTPUT_DIR, os.path.basename(filename))
    return filename # If user provides absolute path, strict isolation might require blocking, but we'll allow for now.
